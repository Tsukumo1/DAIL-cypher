...
class Llama3:
    ...
    def init_model(self):
        self.tokenizer = AutoTokenizer.from_pretrained("path-to/llama3.1_8b", trust_remote_code=True)
        self.model = AutoModelForCausalLM.from_pretrained("path-to/llama3.1_8b", trust_remote_code=True)
        if torch.cuda.is_available():
            self.model = self.model.cuda()

    def ask_llama(self, messages, temperature=0, max_tokens=200):
        # 验证输入格式
